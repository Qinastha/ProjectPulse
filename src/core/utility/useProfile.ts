...
import {setProfileNull, updateProfile} from "../../store/userSlice";

export const useProfileForm = (initialData: ProfileFormData, mode: "create" | "update") => {
    ...
    const validateFormData = () => {
        ...
        if (!addressRegex.test(formData.address.street) || formData.address.street.length === 0) {
            newErrors.street = "Street address cannot start with symbols or be empty.";
            formIsValid = false;
        }
        ...
        if (!addressRegex.test(formData.address.city) || formData.address.city.length === 0) {
            newErrors.city = "City cannot start with symbols or be empty.";
            formIsValid = false;
        }

        if (!zipCodeRegex.test(formData.address.zipCode)) {
            newErrors.zipCode = "Zip code must be numeric and no more than 10 digits.";
            formIsValid = false;
        }
